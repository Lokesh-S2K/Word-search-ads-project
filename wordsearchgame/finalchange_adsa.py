...
#         prompt_surface = font.render(text1, True, pygame.Color('white'))
#         SCREEN.blit(prompt_surface, (input_box.x, input_box.y - 40))
...
def handle_user_input():
    ...
    color = color_inactive
    active = False
    text1 = 'Enter the grid size (at least 14):'
    text = ''
    font = pygame.font.Font(None, 32)
    done = False
                ...
                if active:
                    if event.key == pygame.K_RETURN:
                        try:
                            grid_size = int(text)
                            if grid_size < 14:
                                raise ValueError("Grid size must be at least 14")
                            done = True
...
def display_gameover_win(score, start_time, total_characters):
    ...
    if score == 0:
        message = "Game Over! YOU LOST!!!! You didn't find any words."
    elif score == total_characters:
        message = f"Congratulations! YOU WON !! You found all words with a score of {score}."
    else:
        message = "Game Over! YOU LOST !!! Still words are present."
